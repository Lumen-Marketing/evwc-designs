...
# responsive: the deck becomes a swipeable rail, which is the honest control on
# a touch screen, and every card goes back to full size
rep("""@media (prefers-reduced-motion:reduce){*{animation:none!important;transition:none!important;scroll-behavior:auto!important}}""",
"""@media (max-width:760px){
  ...
  .slide{position:static;flex:0 0 78%;width:auto;height:auto;scroll-snap-align:center;transform:none;opacity:1}
  .slide.far{opacity:1;pointer-events:auto}
  .frame{height:auto}
  .frame>.core{aspect-ratio:3/4;height:auto}
  .frame .core::after{opacity:0}
...
DECK_JS = """(function(){
  /* The deck. One card holds the centre line at full size and its neighbours
     step back either side. Offsets accumulate from each step's scaled width so
     the overlap stays even. Only transform and opacity animate. */
...
print('D2 deck js:', len(done), 'edits ->', ', '.join(done))
